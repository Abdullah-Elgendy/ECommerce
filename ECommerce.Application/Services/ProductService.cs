...
namespace ECommerce.Application.Services
{
    internal class ProductService(IUnitOfWork unitOfWork, IMapper mapper) : IProductService
    {
        ...
        public async Task<Result<PaginatedResult<ProductDto>>> GetAllProductsAsync(ProductQueryParams queryParams, CancellationToken ct = default)
        {
            ...
            var result = new PaginatedResult<ProductDto>(queryParams.PageSize, queryParams.PageIndex, countOfAllProducts, mappedData);
        ...
        {
            var product = await unitOfWork.GetRepository<Product, int>().GetByIdAsync(new ProductWithTypeAndBrandSpec(id), ct);
            if (product is null)
